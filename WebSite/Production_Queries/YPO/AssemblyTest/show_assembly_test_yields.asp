...
<head>
<title>Yields For Assembly Testing</title>
</head>
...
<h2 align="center">Yields For Sub-Assembly Testing</h2>
...
Sql = "SELECT I.IndexID, PN.Family, PN.Category, I.PartNumber, I.SerialNumber, I.Sequence, I.TestResult, StartTime, ResultsID, PN.Plant, PN.Description " & _
      ...
      "WHERE (StartTime BETWEEN " & QueryStartDate & " AND " & QueryEndDate & ") AND I.RecordType = 109 AND I.Sequence = 1 " & _
      "ORDER BY 2,3,4"
...
Sub show_yields_by_family_category()
	...
	Do While Not rs.EOF
		...
		If ( rs("Family") <> Family ) OR ( rs("Category") <> Category ) Then
			...
			Response.Write "<tr>"
			Response.Write "<td><A HREF='show_test_logs.asp?LinkDateStart="& LinkDateStart & "&LinkDateEnd=" & LinkDateEnd & "&Family=" & Family & "'>" & Family & "</A></td>"
			Response.Write "<td><A HREF='show_test_logs.asp?LinkDateStart="& LinkDateStart & "&LinkDateEnd=" & LinkDateEnd & "&Family=" & Family & "&Category=" & Category & "'>" & Category & "</A></td>"
			...
			Response.Write "<tr>"
				...
				For Ctr = 0 To RS.Fields.Count - 1
					...
					ElseIf RS.Fields(Ctr).Name = "IndexID" Then
						Response.Write "<td><A HREF='get_report.asp?ID=" & RS.Fields(Ctr) & "'>"  & RS.Fields(Ctr) & "</A></td>"
					Else
