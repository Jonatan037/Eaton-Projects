...
<head>
<title>9395 Process Capability</title>
</head>
...
Sql = "SELECT I.DBID, I.PartNumber, I.SerialNumber, I.Sequence, I.StartTime, I.TestResult, I.ComputerName, I.ResultsID, P.Family, P.Category " & _
      ...
      "WHERE " & _
         "I.RecordType = 2 AND I.DBID = 2 AND P.Family = '9355' AND P.PartNumber LIKE 'KB%' AND I.Sequence = 1 AND I.TestResult = 'Pass' AND " & _
...
Sql = "SELECT I.DBID, I.PartNumber, I.SerialNumber, I.Sequence, I.StartTime, I.TestResult, I.ComputerName, I.ResultsID, P.Family, P.Category " & _
      "FROM qdms_master_index AS I INNER JOIN qdms_part_numbers AS P ON I.PartNumber = P.PartNumber " & _
      "WHERE " & _
         "I.RecordType = 2 AND I.DBID = 2 AND P.Family = '9395' AND P.Category = 'UPS' AND I.Sequence = 1 AND I.TestResult = 'Pass' AND " & _
         "StartTime BETWEEN " & QueryStartDate & " AND " & QueryEndDate
...
Sub PrintStepInfo(sTestStep)
	...
	With rsTestResults
		...
		Do Until .EOF
			...
			If StepName = "Online Out Vrms Max Load - Out Vrms Ph A" Then 
				blnGetData = TRUE
			End If

			If StepName = "Online Out Vrms Max Load - Out Vrms Ph B" Then 
				blnGetData = TRUE
			End If

			If StepName = "Online Out Vrms Max Load - Out Vrms Ph C" Then 
				blnGetData = TRUE
			End If

			If StepName = "Online Out Freq - Output Freq Ph A" Then 
				blnGetData = TRUE
			End If

			If StepName = "Online Out Freq - Output Freq Ph B" Then 
				blnGetData = TRUE
			End If

			If ( StepName = "Online Out Freq - Output Freq Ph C" ) AND ( Trim(.Fields("Maximum")) <> "63" ) Then 
				blnGetData = TRUE
			End If
