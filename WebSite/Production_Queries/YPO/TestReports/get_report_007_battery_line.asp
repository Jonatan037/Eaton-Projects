...
<head>
<title>Battery Line Production Test Report</title>
...
Dim Conn			'Battery database connection
...
Set rs = Conn.Execute("SELECT ID, PartNumber, SerialNumber, StartTime, StopTime,  Seq,  Badge, IIF(Results, 'Pass', 'Fail') AS [Result] FROM Index WHERE ID = " & lResultsID)

Set rsFailureInfo = Conn.Execute("SELECT * FROM [View - Failure Information] WHERE ID = " & lResultsID)

Set rsMeasurements = Conn.Execute("SELECT [Parameter Name], [Data], [Low Limit],[High Limit], IIF(Status, 'Pass', 'Fail') AS [Result] FROM [Measurements View] WHERE ID = " & lResultsID)
...
Else
	...
	If NOT rsMeasurements.BOF Then

		Response.Write "<table cellspacing=0 cellpadding=2 border=0>"
		Response.Write "<caption>Measurement</caption>"

		For Each fld In rsMeasurements.Fields
