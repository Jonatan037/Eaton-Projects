...
async function main() {
  console.log("🏎️  Seeding F1 Tracks...\n");
  ...
  for (const track of F1_TRACKS) {
    ...
    if (existing) {
      ...
      console.log(`  ✅ Updated: ${track.name} (${track.countryCode})`);
    } else {
      // Create new track
      ...
      console.log(`  ➕ Created: ${track.name} (${track.countryCode})`);
    }
  }

  console.log(`\n🏁 Seeded ${F1_TRACKS.length} F1 tracks!`);
  ...
  tracks.forEach((t) => {
    const flag = countryFlags[t.countryCode || ""] || "🏁";
