        ...
        <div className="relative border-b border-white/10 overflow-hidden">
          ...
          <div className="container mx-auto px-4 py-10 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
              {[
                { icon: Users, value: league.memberships.length, label: t('members'), color: '#2ECC71' },
                { icon: Flag, value: league.teams.length, label: t('teams'), color: '#3B82F6' },
                { icon: Trophy, value: totalDrivers, label: t('drivers'), color: '#F59E0B' },
                { icon: Calendar, value: `${completedRounds.length}/${league.rounds.length}`, label: 'Rounds', color: '#8B5CF6' },
              ].map((stat, i) => (
                <div key={i} className="relative overflow-hidden rounded-xl bg-white/[0.03] border border-white/10 p-4">
        ...
        <div className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
          <Tabs defaultValue="overview" className="space-y-6">
            <TabsList className="bg-white/5 border border-white/10 p-1">
              <TabsTrigger value="overview" className="data-[state=active]:bg-[#2ECC71] data-[state=active]:text-white">{t('overview')}</TabsTrigger>
              <TabsTrigger value="schedule" className="data-[state=active]:bg-[#2ECC71] data-[state=active]:text-white">{t('schedule')}</TabsTrigger>
              <TabsTrigger value="standings" className="data-[state=active]:bg-[#2ECC71] data-[state=active]:text-white">{t('standings')}</TabsTrigger>
              <TabsTrigger value="teams" className="data-[state=active]:bg-[#2ECC71] data-[state=active]:text-white">{t('teams')}</TabsTrigger>
              <TabsTrigger value="results" className="data-[state=active]:bg-[#2ECC71] data-[state=active]:text-white">{t('results')}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="overview" className="space-y-6">
              <div className="grid gap-6 md:grid-cols-2">
                  ...
                  {nextRace ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 mt-4">
                        {nextRace.hasQuali && (
                          <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">{tRound('qualifying')}</Badge>
                        )}
                        {nextRace.hasSprint && (
                          <Badge className="bg-orange-500/20 text-orange-400 border-orange-500/30">{tRound('sprint')}</Badge>
                        )}
                        <Badge className="bg-[#2ECC71]/20 text-[#2ECC71] border-[#2ECC71]/30">{tRound('race')}</Badge>
                      </div>
                    </div>
                  ) : (
                ...
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] border border-white/10 p-6">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-white">Season Progress</h3>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-sm mb-2">
                        <span className="text-gray-400">Races Completed</span>
                        <span className="text-white font-medium">{completedRounds.length}/{league.rounds.length}</span>
                      </div>
                      <div className="h-3 rounded-full bg-white/10 overflow-hidden">
                        <div 
                          className="h-full rounded-full bg-gradient-to-r from-[#2ECC71] to-[#27AE60]"
                          style={{ width: `${league.rounds.length > 0 ? (completedRounds.length / league.rounds.length) * 100 : 0}%` }}
                        />
                      </div>
                    </div>
                    <Button 
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400">Get insights, predictions, and stats powered by AI</p>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-white">{t('teams')}</h3>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  {league.teams.slice(0, 4).map((team) => (
                    ...
                    >
                      ...
                      />
                      <h4 className="font-semibold text-white ml-2">{team.name}</h4>
            ...
            <TabsContent value="schedule">
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] border border-white/10">
                <div className="p-6 border-b border-white/10">
                  ...
                  <p className="text-gray-400 mt-1">All rounds for this championship</p>
                </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          ...
                          <div className="flex gap-1 mt-1 justify-end">
                            {round.hasSprint && (
                              <Badge className="text-xs bg-orange-500/20 text-orange-400 border-orange-500/30">Sprint</Badge>
                            )}
                          </div>
                        </div>
                        <Badge 
            ...
            <TabsContent value="standings">
              <div className="grid gap-6 md:grid-cols-2">
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] border border-white/10 p-6">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    <Trophy className="h-5 w-5 text-[#F59E0B]" />
                    <h3 className="text-lg font-semibold text-white">{tStandings('driverStandings')}</h3>
                  </div>
                ...
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] border border-white/10 p-6">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-white">{tStandings('constructorStandings')}</h3>
                  </div>
                  <p className="text-gray-500">
                    Standings will appear here once results are entered.
                  </p>
                    ...
                    <div className="p-5">
                      <div className="flex items-center gap-3 mb-4">
                        <div 
                          ...
                          style={{ backgroundColor: `${team.primaryColor || '#2ECC71'}40` }}
                        >
                      ...
                      <div className="space-y-2">
                        {team.drivers.map((driver) => (
                          ...
                          >
                            ...
                            {driver.isReserve && (
                              <Badge className="text-xs bg-white/10 text-gray-400 border-white/10">Reserve</Badge>
                            )}
                          </div>
                        ))}
                        {team.drivers.length === 0 && (
                          <p className="text-sm text-gray-500 py-2">No drivers assigned</p>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <TabsContent value="results">
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] border border-white/10">
                <div className="p-6 border-b border-white/10">
                  ...
                  <p className="text-gray-400 mt-1">Race results for completed rounds</p>
                </div>
                {completedRounds.length === 0 ? (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-sm text-gray-600 mt-1">Results will appear after races are completed.</p>
                  </div>
                ) : (
                  <div className="divide-y divide-white/5">
