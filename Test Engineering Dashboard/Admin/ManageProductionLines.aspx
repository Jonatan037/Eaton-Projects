<%@ Page Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="ManageProductionLines.aspx.cs" Inherits="TED_Admin_ManageProductionLines" %>
...
<asp:Content ID="TitleC" ContentPlaceHolderID="TitleContent" runat="server">Production Line DB - Admin</asp:Content>
<asp:Content ID="HeadC" ContentPlaceHolderID="HeadContent" runat="server">
  ...
  table.data-table th:nth-child(1), table.data-table td:nth-child(1) { min-width:60px; text-align:center; }
  table.data-table th:nth-child(2), table.data-table td:nth-child(2) { min-width:120px; text-align:center; }
  table.data-table th:nth-child(3), table.data-table td:nth-child(3) { min-width:250px; text-align:center; }
  table.data-table th:nth-child(4), table.data-table td:nth-child(4) { min-width:280px; text-align:center; }
  table.data-table th:nth-child(5), table.data-table td:nth-child(5) { min-width:180px; text-align:center; }
  table.data-table th:nth-child(6), table.data-table td:nth-child(6) { min-width:180px; text-align:center; }
  table.data-table th:nth-child(7), table.data-table td:nth-child(7) { min-width:150px; text-align:center; }
  ...
  /* Light theme overrides for Select2 */
  html.theme-light .select2-container--default .select2-selection--single,
  html[data-theme='light'] .select2-container--default .select2-selection--single {
    background: #fff !important;
    border: 1px solid rgba(0,0,0,.14) !important;
  }
  html.theme-light .select2-dropdown,
  html[data-theme='light'] .select2-dropdown {
    background: #ffffff !important;
    border: 1px solid rgba(0,0,0,.14) !important;
  }
  html.theme-light .select2-container--default .select2-results__option,
  html[data-theme='light'] .select2-container--default .select2-results__option {
    background: #ffffff !important;
    color: #1f2530 !important;
  }
  html.theme-light .select2-container--default .select2-results__option--highlighted[aria-selected],
  html[data-theme='light'] .select2-container--default .select2-results__option--highlighted[aria-selected] {
    background: #f3f7ff !important;
    color: #0b2960 !important;
  }
  html.theme-light .select2-container--default .select2-results__option[aria-selected=true],
  html[data-theme='light'] .select2-container--default .select2-results__option[aria-selected=true] {
    background: #e6f0ff !important;
    color: #0b2960 !important;
  }
  html.theme-light .select2-search--dropdown .select2-search__field,
  ...
  <script>
    ...
    function confirmDelete() {
      return confirm('Are you sure you want to delete this production line?');
    }

    // Initialize Select2 on supervisor dropdowns
    function initSelect2() {
      $('select[id^="supervisor_"]').each(function() {
...
<asp:Content ID="MainC" ContentPlaceHolderID="MainContent" runat="server">
  <div class="admin-grid">
    ...
    <div class="admin-container">
      <uc1:AdminHeader ID="AdminHeader1" runat="server" Title="Production Line Database" />
      <div id="globalToast" class="global-toast"></div>

      <div class="toolbar">
        <div class="field">
          <label>Search</label>
          <asp:TextBox ID="txtSearch" runat="server" placeholder="Name, E-Number, Email, Department, Role" AutoPostBack="True" OnTextChanged="txtSearch_TextChanged" />
        </div>
        <div class="field">
          <label>Plant</label>
          <asp:DropDownList ID="ddlPlantFilter" runat="server" AutoPostBack="True" OnSelectedIndexChanged="ddlPlantFilter_SelectedIndexChanged">
            <asp:ListItem Value="">All Plants</asp:ListItem>
            <asp:ListItem Value="YPO">YPO</asp:ListItem>
            <asp:ListItem Value="RPO">RPO</asp:ListItem>
      ...
      <div class="table-wrap">
        <asp:Repeater ID="rptLines" runat="server" OnItemCommand="rptLines_ItemCommand">
          ...
          <ItemTemplate>
            <tr id='row_<%# Eval("ProductionLineID") %>'>
              <td><input type="text" class="inline-input" value='<%# Eval("ProductionLineID") %>' disabled /></td>
              <td>
                <select class="inline-select" id='plant_<%# Eval("ProductionLineID") %>' name='plant_<%# Eval("ProductionLineID") %>'>
                  <option value="" <%# string.IsNullOrEmpty(Eval("Plant") as string) ? "selected" : "" %>></option>
                  <asp:Repeater ID="rptPlants" runat="server" DataSource='<%# GetPlants() %>'>
                    <ItemTemplate>
                      <option value='<%# Eval("Plant") %>' <%# Container.Parent.Parent is RepeaterItem && Eval("Plant").ToString() == DataBinder.Eval(((RepeaterItem)Container.Parent.Parent).DataItem, "Plant") as string ? "selected" : "" %>><%# Eval("Plant") %></option>
                    </ItemTemplate>
                  </asp:Repeater>
                </select>
              </td>
              <td><input type="text" class="inline-input" id='lineName_<%# Eval("ProductionLineID") %>' name='lineName_<%# Eval("ProductionLineID") %>' value='<%# Eval("ProductionLineName") %>' /></td>
              <td>
                <select class="inline-select" id='supervisor_<%# Eval("ProductionLineID") %>' name='supervisor_<%# Eval("ProductionLineID") %>'>
                  <option value="" <%# string.IsNullOrEmpty(Eval("CurrentSupervisor") as string) ? "selected" : "" %>></option>
                  <asp:Repeater ID="rptSupervisors" runat="server" DataSource='<%# GetSupervisors() %>'>
                    <ItemTemplate>
                      <option value='<%# Eval("Email") %>' <%# Container.Parent.Parent is RepeaterItem && Eval("Email").ToString() == DataBinder.Eval(((RepeaterItem)Container.Parent.Parent).DataItem, "CurrentSupervisor") as string ? "selected" : "" %>><%# Eval("Email") %></option>
                    </ItemTemplate>
                  </asp:Repeater>
                </select>
              </td>
              ...
              <td>
                <div class="tbl-actions">
                  <asp:LinkButton runat="server" CommandName="Save" CommandArgument='<%# Eval("ProductionLineID") %>' CssClass="save-btn" ToolTip="Save changes" OnClientClick='<%# "showRowMsg(\"row_" + Eval("ProductionLineID") + "\", \"Saving...\", \"success\"); return true;" %>'>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>
                  </asp:LinkButton>
                  <asp:LinkButton runat="server" CommandName="Delete" CommandArgument='<%# Eval("ProductionLineID") %>' CssClass="delete-btn" ToolTip="Delete line" OnClientClick="return confirmDelete();">
