...
public partial class Tracks_Reports_DeviceHistory_Device_History : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        gvHistory.RowDataBound += gvHistory_RowDataBound;
    ...
    protected void gvHistory_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            // Find the Status cell by header name
            int statusColumnIndex = -1;

            // Only determine column index once
            if (statusColumnIndex == -1)
            {
                for (int i = 0; i < e.Row.Cells.Count; i++)
                {
                    if (gvHistory.Columns[i].HeaderText == "Status")
                    {
                        statusColumnIndex = i;
                        break;
                    }
            ...
            {
                TableCell statusCell = e.Row.Cells[statusColumnIndex];
                string statusText = statusCell.Text.Trim().ToUpper();

                if (statusText == "PASSED" || statusText == "PASS")
                {
                    statusCell.BackColor = Color.LightGreen;
                }
