...
public partial class NCRs_Standard_Reports_ShowIssuePercentages : System.Web.UI.Page
{
    ...
    private void GetYields()
    {
        ...
        sql = "SELECT " +
                  ...
                  "Count(MASTER_INDEX.FAILED) - Sum(Abs(MASTER_INDEX.FAILED)) AS [Without Issues], " +
                  "Sum(Abs(MASTER_INDEX.FAILED)) AS [With Issues], " +
                  "FORMAT( 1 - ( Sum(Abs(MASTER_INDEX.FAILED)) / Count(MASTER_INDEX.FAILED) ), 'P' ) AS [Percentage Without Issues] " +
               "FROM MASTER_INDEX " +
               "WHERE " + date_range + " " +
